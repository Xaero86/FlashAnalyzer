...
DefineBitsLosslessTag::DefineBitsLosslessTag(const char* source, uint32_t headerLength, uint32_t dataLength) :
 ImageTag(source, DEFINE_BITS_LOSSLESS_TAG, headerLength, dataLength),
 ...
 _unzipDataSize(0)
{
    ...
    _unzipDataSize = _bitmapColorTableSize * 3 + (_bitmapWidth * pixelSizeBytesSWF(_bitmapFormat) + paddingWidthSWF) * _bitmapHeight;
...
#define HEADER_SIZE 14 + 40
    ...
    {
        // _bitmapFormat == BitmapFormat::B_15BITS : not tested, may not work
        memcpy(&_bitmapData[bitmapDataIndex], _unzipData, _unzipDataSize);
    }
...
void DefineBitsLosslessTag::extract(QDataStream &outputStream)
{
